...
/**
 ...
 \sa printf()
*/
    void text(cv::Mat &theWhere, int theX, int theY, const cv::String &theText, double theFontScale = 0.4,
              unsigned int theColor = 0xCECECE);
...
*/
    void
    printf(cv::Mat &theWhere, int theX, int theY, double theFontScale, unsigned int theColor, const char *theFmt, ...);
...
*/
    template<typename T>
    bool
    trackbar(cv::Mat &theWhere, int theX, int theY, int theWidth, T *theValue, T theMin, T theMax, int theSegments = 1,
...
*/
    template<typename T>
    // T can be any float type (float, double, long double)
    bool
    trackbar(int theWidth, T *theValue, T theMin, T theMax, int theSegments = 1, const char *theLabelFormat = "%.1Lf",
...
// You should probably not be using anything from here.
    namespace internal {
        ...
        double
        counter(cvui_block_t &theBlock, int theX, int theY, double *theValue, double theStep, const char *theFormat);

        void
        window(cvui_block_t &theBlock, int theX, int theY, int theWidth, int theHeight, const cv::String &theTitle);
        ...
        template<typename T>
        // T can be any floating point type (float, double, long double)
        ...
        template<typename T>
        // T can be any numeric type (int, double, unsigned int, etc)
        bool
        trackbar(cv::Mat &theWhere, int theX, int theY, int theWidth, T *theValue, const TrackbarParams &theParams);
...
// Internal namespace that contains all rendering functions.
    namespace render {
        ...
        void trackbarPath(cvui_block_t &theBlock, int theState, cv::Rect &theShape, double theValue,
                          const internal::TrackbarParams &theParams, cv::Rect &theWorkingArea);

        void trackbarSteps(cvui_block_t &theBlock, int theState, cv::Rect &theShape, double theValue,
                           const internal::TrackbarParams &theParams, cv::Rect &theWorkingArea);
        ...
        void trackbarSegments(cvui_block_t &theBlock, int theState, cv::Rect &theShape, double theValue,
                              const internal::TrackbarParams &theParams, cv::Rect &theWorkingArea);
    ...
    template<typename num_type>
    bool
    trackbar(cv::Mat &theWhere, int theX, int theY, int theWidth, num_type *theValue, num_type theMin, num_type theMax,
             int theSegments, const char *theLabelFormat, unsigned int theOptions, num_type theDiscreteStep) {
        internal::TrackbarParams aParams = internal::makeTrackbarParams(theMin, theMax, theDiscreteStep, theSegments,
                                                                        theLabelFormat, theOptions);
    ...
    bool trackbar(int theWidth, num_type *theValue, num_type theMin, num_type theMax, int theSegments,
                  const char *theLabelFormat, unsigned int theOptions, num_type theDiscreteStep) {
        internal::TrackbarParams aParams = internal::makeTrackbarParams(theMin, theMax, theDiscreteStep, theSegments,
                                                                        theLabelFormat, theOptions);
...
// that is shared among components/functions
    namespace internal {
        ...
        inline long double
        trackbarXPixelToValue(const TrackbarParams &theParams, cv::Rect &theBounding, int thePixelX) {
            long double ratio = (thePixelX - (long double) (theBounding.x + gTrackbarMarginX)) /
        ...
        inline int trackbarValueToXPixel(const TrackbarParams &theParams, cv::Rect &theBounding, long double theValue) {
            ...
            long double thePixelsX = (long double) theBounding.x + gTrackbarMarginX +
...
// that actually render each one of the UI components
    namespace render {
        ...
        void button(cvui_block_t &theBlock, int theState, cv::Rect &theShape, const cv::String &theLabel) {
            ...
            cv::rectangle(theBlock.where, theShape,
                          theState == OUT ? cv::Scalar(0x42, 0x42, 0x42) : (theState == OVER ? cv::Scalar(0x52, 0x52,
                                                                                                          0x52)
                                                                                             : cv::Scalar(0x32, 0x32,
                                                                                                          0x32)),
                          CVUI_FILLED);
        ...
        int putTextCentered(cvui_block_t &theBlock, const cv::Point &position, const std::string &text) {
            ...
            cv::putText(theBlock.where, text, positionDecentered, cv::FONT_HERSHEY_SIMPLEX, aFontScale,
        ...
        void buttonLabel(cvui_block_t &theBlock, int theState, cv::Rect theRect, const cv::String &theLabel,
            ...
            if (!aLabel.hasShortcut) {
                ...
                cv::line(theBlock.where, cv::Point(aStart, aPos.y + 3), cv::Point(aEnd, aPos.y + 3), aColor, 1,
                         CVUI_ANTIALISED);
            }
        }
        ...
        void counter(cvui_block_t &theBlock, cv::Rect &theShape, const cv::String &theValue) {
            ...
            cv::putText(theBlock.where, theValue, aPos, cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0xCE, 0xCE, 0xCE), 1,
                        CVUI_ANTIALISED);
        }
        ...
        void window(cvui_block_t &theBlock, cv::Rect &theTitleBar, cv::Rect &theContent, const cv::String &theTitle) {
            ...
            cv::putText(theBlock.where, theTitle, aPos, cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0xCE, 0xCE, 0xCE), 1,
                        CVUI_ANTIALISED);
        ...
        void rect(cvui_block_t &theBlock, cv::Rect &thePos, unsigned int theBorderColor, unsigned int theFillingColor) {
            ...
            if (aHasFilling) {
                if (aFilling[3] == 0x00) {
                    ...
                    cv::addWeighted(aOverlay, aAlpha, theBlock.where(aClippedRect), 1.00 - aAlpha, 0.0,
    ...
    void
    init(const cv::String theWindowNames[], size_t theHowManyWindows, int theDelayWaitKey, bool theCreateNamedWindows) {
    ...
    bool
    checkbox(cv::Mat &theWhere, int theX, int theY, const cv::String &theLabel, bool *theState, unsigned int theColor) {
        internal::gScreen.where = theWhere;
        return internal::checkbox(internal::gScreen, theX, theY, theLabel, theState, theColor);
    }

    void
    text(cv::Mat &theWhere, int theX, int theY, const cv::String &theText, double theFontScale, unsigned int theColor) {
        internal::gScreen.where = theWhere;
